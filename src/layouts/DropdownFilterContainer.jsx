...
function DropdownFilterContainer({ dropdownName, mainDataLength, content }) {
 return (
  <div className="max-md:fixed max-md:top-[25vh] max-md:left-[5vw] md:absolute left-0 md:w-full md:top-[4.6rem] z-10">
   ...
   <div className={`bg-background-100 rounded-md overflow-y-auto overflow-scroll-dropdown-filter shadow-md closest-${_.camelCase(dropdownName)}-filter max-md:w-[90vw] ${mainDataLength > 8 && 'max-h-[60vh] pl-2 pr-2'} py-2 px-2`}>
    {content}
   </div>
  </div>
 )
