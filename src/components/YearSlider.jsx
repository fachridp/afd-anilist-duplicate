...
function YearSlider() {
 ...
 const handleYearSliderAfterChange = useCallback((value) => {
  ...
  yearSliderAfterChange !== value && dispatch(setFilterSlider({ keyFilter, valueFilter }));
 }, [dispatch, yearSliderAfterChange]);

 const handleResetYearSlider = () => {
  const valueFilter = SLIDERS_VALUES.year.defaultValue;

  dispatch(setFilterSlider({ keyFilter: 'year', valueFilter }));
  dispatch(setFilterSlider({ keyFilter: 'yearChange', valueFilter }));
 }

 const sliderYearHasChanged = JSON.stringify(yearSliderAfterChange) !== JSON.stringify(SLIDERS_VALUES.year.defaultValue)
 ...
 return (
  <div className="hidden lg:block slider-container mt-8 row-start-2">
   <div className="font-semibold flex justify-between items-center">
    ...
    {sliderYearHasChanged && (
     <div className="flex items-start gap-x-1 closest-button-filter-large">
      <p className="text-gray-700 text-xs">{yearSliderAfterChange.join('-')}</p>
