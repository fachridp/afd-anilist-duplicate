...
function GenresFilter() {
 ...
 const handleSearchGenresFilter = (event) => {
  const keyFilter = 'searchGenresFilter';
  const valueFilter = event.target.value;
  dispatch(setSearchingFilter({ keyFilter, valueFilter }));
 }

 const handleOpenDropdownGenresFilter = () => {
 ...
 return (
  <div className="w-[10.625rem] relative max-lg:snap-start" data-filtercamelcase="genres">
   <label htmlFor="genres">
    ...
    <div className="relative">
     <input
      ...
      type="text"
      onClick={handleOpenDropdownGenresFilter}
      name="genres"
      id="genres"
      value={searchedGenresFilter}
      onChange={handleSearchGenresFilter}
      placeholder={selectedGenresFilter.length > 0 ? '' : 'Any'}
      className={`bg-background-100 closest-genres-filter shadow-md w-full py-[0.6875rem] pr-8 pl-4 placeholder:font-semibold placeholder:text-gray-600 lg:placeholder:font-bold text-gray-700 focus:outline-none cursor-pointer text-[0.8125rem] font-normal rounded-md lg:cursor-text peer ${selectedGenresFilter.length > 0 && 'not-focus:text-white'}`}
   ...
   <span className="absolute flex items-center inset-y-0 pr-3 top-[1.42rem] right-0 cursor-pointer">
    {selectedGenresFilter.length > 0 ? (
     <svg data-filter-name="genres" onClick={handleClearSelectedFilterGenres} className="w-3 fill-gray-500 hover:fill-gray-600 ease-in-out duration-75 py-2 lg:w-3.5" aria-hidden="true" focusable="false" role="img" data-icon="cross" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 20 20">
