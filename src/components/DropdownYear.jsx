...
// Import RTK features
import { setFilterRadio, setSearchingFilter } from "../features/selectedFiltersManager";
...
export default function DropdownYear({ dropdownName, mainData }) {
 ...
 const handlehandleSelectFilterYear = (event) => {
   ...
   <div>
    {searchedYearResult && searchedYearResult.map(data => (
     <label key={data.id} htmlFor={`${dropdownName} ${data.name}`} className="flex justify-between items-center cursor-pointer p-2 group hover:bg-background-200 closest-year-filter duration-75 ease-in-out rounded-md relative my-1">
      ...
      <input type="radio" value={data.name} checked={data.name.toString() === dropdownYearValue} onChange={handlehandleSelectFilterYear} className="peer appearance-none" name={`${dropdownName} ${data.name}`} id={`${dropdownName} ${data.name}`} />
