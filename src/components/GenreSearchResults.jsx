...
function GenreSearchResults({ isAdvancedFilter, htmlfor }) {
 const dispatch = useDispatch();

 // RTK Store
 const checkboxFilterValues = useSelector((state) => state.selectedFiltersManager.checkboxDropdownFilters.genres);
 const searchedGenresFilter = useSelector((state) => state.selectedFiltersManager.searchGenresFilter) // User are typing their search queries.
 ...
 return (
  <>
   {filteredGenres.map(data => (
    <label key={data.id} data-isadvancedfilter={isAdvancedFilter} htmlFor={`${htmlfor}-${data.name}-search-result`} className="flex justify-between items-center cursor-pointer data-[isadvancedfilter=false]:hover:bg-background-200 duration-75 ease-in-out rounded-md data-[isadvancedfilter=false]:p-2 mt-1 group relative data-[isadvancedfilter=false]:flex-row-reverse">
     <input type="checkbox" value={data.name} checked={checkboxFilterValues.includes(data.name)} onChange={handleSelectFilterCheckbox} className="peer appearance-none" name={`${htmlfor}-${data.name}-search-result`} id={`${htmlfor}-${data.name}-search-result`} />
...
GenreSearchResults.propTypes = {
 isAdvancedFilter: PropTypes.string,
 htmlfor: PropTypes.string,
 index: PropTypes.number,
}

export default memo(GenreSearchResults);